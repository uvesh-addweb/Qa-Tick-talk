...
button_ids = ['//*[@id="table-actions"]/a', '//*[@id="add-lead"]', '//*[@id="table-actions"]/div/button']
add_button = "null"

for button_id in button_ids:
    button = driver.find_element(By.XPATH, f"{button_id}")

    if button.text == "Add Lead":
        add_button = driver.find_element(By.XPATH, f"{button_id}")

    if button.is_enabled():
        print(f"{button.text} button is Active")
    else:
        print(f"{button.text} button is Inactive")

# Check the color of the Add lead button
background_color = add_button.value_of_css_property("background-color")

if background_color == "rgba(29, 130, 245, 1)":
    print(f"The background-color {background_color} of Add leads button is matching ")
